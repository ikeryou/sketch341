...
export class BlockList extends MyObject3D {
  ...
  constructor(opt: {num:number}) {
    ...
    const geoFillA = new ConeGeometry(0.5, 0.5, 28, 8);
    const geoFillB = new ConeGeometry(0.25, 3, 16, 16);
  ...
  protected _update():void {
    ...
    this._item.forEach((val,i) => {
      ...
      let radius = sw * Func.instance.val(0.25, 0.15);

      if(num <= 10) {
        radius *= Util.instance.map(this._scroll, 1, 0.75, 0, scrollHeight - sh);
      } else {
        radius *= Util.instance.map(this._scroll, 0.75, 2.5, 0, scrollHeight - sh);
      }
      ...
      const move = 0.2;
      if(num > 10) val.position.z = Util.instance.map(this._scroll, sh * move, sh * -move, 0, scrollHeight - sh);
      if(num <= 10) val.position.z = Util.instance.map(this._scroll, sh * -move, sh * move, 0, scrollHeight - sh);

      val.rotation.z = Math.atan2(dy, dx);
      if(num > 10) val.rotation.z += Util.instance.radian(Util.instance.map(this._scroll, 0, 720, 0, scrollHeight - sh));
    })
    ...
    this._mat.forEach((val) => {
      ...
      uni.time.value += 1;
      uni.alpha.value = Util.instance.map(this._scroll, 1, 0, 0, scrollHeight - sh);
    })
  }
}